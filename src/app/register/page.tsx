...
if (!agree) {
  alert("Vui lòng đồng ý chính sách và điều khoản dịch vụ");
  return;
}
...
return ( <main style={page}> <div style={bgCircle1}></div> <div style={bgCircle2}></div> <div style={dotBox}></div>
  ...
  <div style={card}>
    ...
    <input
      ...
      placeholder="Ví dụ: Thien2309"
      style={input}
    />

    <label style={label}>✉️ Địa chỉ Email</label>
    <input
      value={email}
    ...
    <label style={label}>🎁 Mã giới thiệu (không bắt buộc)</label>
    <input
      value={referralCode}
      ...
      style={{
        ...input,
        opacity: isReferralFromLink ? 0.75 : 1,
        cursor: isReferralFromLink ? "not-allowed" : "text",
      }}
    />

    {isReferralFromLink && (
      <div style={refNotice}>✅ Đã nhận mã giới thiệu từ link</div>
    )}
    ...
    <label style={agreeRow}>
      ...
      />
      <span>
        Đồng ý với <b style={{ color: "#22d3ee" }}>Chính sách</b> và{" "}
        <b style={{ color: "#ec4899" }}>Điều khoản dịch vụ</b>
      </span>
    </label>
